...
bot.on('ready', () => {
    if (bot.user.bot) {
        logger.severe(`X-Bot est un selfbot, mais vous avez entré un token bot. Veuillez suivre les instructions dans ${chalk.green('README')} et ré-entrez votre token d'utilisateur.`);
        process.exit(666);
    }
...
bot.on('disconnect', event => {
    if (event.code === 1000) {
        ...
        logger.severe(`Échec de l'authentification avec Discord. Veuillez suivre les instructions dans ${chalk.green('README')} et ré-entrez votre token d'utilisateur.`);
        process.exit(666);
    } else {
